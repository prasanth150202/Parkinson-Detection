...
def load_image():
    ...
    if file_path:
        ...
        result_label.config(text=result, justify='center', foreground=color1, font="Copperplate Gothic Bold")


# Create a GUI window
root = tk.Tk()
root.title("Parkinson's Waves Detection")
...
content_label = ttk.Label(root, text="Parkinson's Disease Wave Predection ", font=("Copperplate Gothic Bold", 20),foreground="blue")
...
Here's how the wave prediction typically works in a medical context

UPLOAD image of wave drawing of report that scanned by our software with 90% accuracy this solve the case
...
# Load the reference images (10 Parkinson's and 10 Healthy for Waves)
parkinson_images = [
    resize(io.imread('C:/Users/HP/PycharmProjects/pythonProject/parkison_wave/parkinson_wave{}.jpg'.format(i)),
           (256, 256), anti_aliasing=True) for i in range(1, 5)]
healthy_images = [
    resize(io.imread('C:/Users/HP/PycharmProjects/pythonProject/healthy_wave/healthy_wave{}.jpg'.format(i)),
           (256, 256), anti_aliasing=True) for i in range(1, 5)]
